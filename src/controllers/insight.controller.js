...
class InsightController {
  ...
  async getRegionInsights(req, res) {
    const { region, timeRange = '7d' } = req.query;
    ...
    try {
      ...
      // Fetch emotion data for the region - improved matching
      const emotions = await Emotion.find({
        $or: [
          { 'location.country': { $regex: region, $options: 'i' } },
          { 'location.city': { $regex: region, $options: 'i' } },
          { 'location.region': { $regex: region, $options: 'i' } },
          // Also check if region contains the country/city name
          { 'location.country': { $regex: region.split(',')[0]?.trim(), $options: 'i' } },
        ...
        privacy: 'public'
        // Temporarily removed date filter to test
  ...
  async getEmotionTrends(req, res) {
    const { region, emotion, days = 7 } = req.query;
